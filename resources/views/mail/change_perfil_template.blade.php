                                ...
                                <table class="inner-body" align="center" width="570" cellpadding="0" cellspacing="0">
                                    ...
                                    <tr>
                                        <td class="content-cell">
                                            <h3>Olá, aqui é {{ $data['name'] }}</h3>
                                            <p>Sou o Cliente com o número {{ $data['sap_id'] }}.</p>
                                            <p>Eu gostaria de efetuar a(s) seguinte(s) alterações no meu perfil:</p>
                                            <ul>
                                                @isset($data['morada']) <li>Morada: {{$data['morada']}} </li>  @endisset
                                                @isset($data['cod_postal']) <li>Cód. Postal: {{$data['cod_postal']}} </li> @endisset
                                                @isset($data['localidade']) <li>Localidade: {{$data['localidade']}} </li> @endisset
                                                @isset($data['telefone']) <li>Telefone: {{$data['telefone']}} </li> @endisset
                                                @isset($data['telemovel']) <li>Telemóvel: {{$data['telemovel']}} </li> @endisset
                                            </ul>
